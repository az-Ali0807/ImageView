...
const CreateImage: React.FC = () => {
  ...
  const handleDescription = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim().length === 0) {
      ...
      setErrors(prev => ({ ...prev, description: "Minium 3 characters required" }));
    } else {
  ...
  const handelCreate = async () => {
    ...
    try {
      const result = await axios.post(`${API_URL}/api/images/create`, formData, {
        headers: {
  ...
  return (
    <div className="create-card">
      <div className='container'>
        ...
        />
        {errors.file && <p style={{ color: 'red', margin: '0'}}>{ errors.file }</p>}
